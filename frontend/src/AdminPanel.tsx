import React, { useState, useEffect, useRef } from 'react';
import axios from 'axios';
import { NameFields, EmbgModule, DiagnosisModule, PhoneInput, OperationInput, SearchInput } from './Components/PatientComponents';
import PatientTable from './Components/PatientTable';
import { Patient, PatientCounts } from './types';
import 'bootstrap/dist/css/bootstrap.min.css';
import './App.css';
import './Components/Style/PatientsInput.css';
import './Components/Style/PatientsTable.css';

interface FormData {
    first_name: string;
    last_name: string;
    phone: string;
    diagnosis: string;
    operation: number | null;
    embg: string;
}

interface NameFieldsRef {
    focusFirst: () => void;
}

function AdminPanel() {
    const [counts, setCounts] = useState<PatientCounts>({ call: 0, waiting: 0, trash: 0 });
    const [patients, setPatients] = useState<Patient[]>([]);
    const [searchTerm, setSearchTerm] = useState<string>("");
    const [activeTab, setActiveTab] = useState<'call' | 'waiting' | 'trash'>('call');
    const [editingField, setEditingField] = useState<{ id: number | null, field: keyof Patient | null }>({ id: null, field: null });
    const [lastSuccess, setLastSuccess] = useState<Patient | null>(null);
    const [formData, setFormData] = useState<FormData>({
        first_name: '', last_name: '', phone: '', diagnosis: '', operation: null, embg: ''
    });

    const nameRef = useRef<NameFieldsRef>(null);

    const handleLogout = () => {
        localStorage.removeItem('doctorToken');
        window.location.href = '/login';
    };

    const fetchCounts = () => {
        axios.get<PatientCounts>('http://127.0.0.1:8000/api/patients/counts/')
            .then(res => setCounts(res.data))
            .catch(err => console.error(err));
    };

    const fetchPatients = (search = "", tab = "call") => {
        let url = `http://127.0.0.1:8000/api/patients/`;
        url += search ? `?search=${encodeURIComponent(search)}` : `?tab=${tab}`;
        axios.get<Patient[]>(url)
            .then(res => setPatients(res.data))
            .catch(err => console.error(err));
    };

    useEffect(() => {
        const delay = setTimeout(() => {
            fetchPatients(searchTerm, activeTab);
            fetchCounts();
        }, 300);
        return () => clearTimeout(delay);
    }, [searchTerm, activeTab]);

    const handleNextField = (e: any) => {
        if (!e?.target) return;
        const form = e.target.closest('form') || e.target.closest('.table-container');
        if (!form) return;
        const elements = Array.from(form.querySelectorAll('input, select, textarea, button')) as HTMLElement[];
        const currentIndex = elements.indexOf(e.target);
        if (elements[currentIndex + 1]) elements[currentIndex + 1].focus();
    };

    const handleLiveEdit = (pId: number, field: keyof Patient, value: any) => {
        setPatients(prev => prev.map(p => p.id === pId ? { ...p, [field]: value } : p));
    };

    const saveField = (p: Patient) => {
        axios.put(`http://127.0.0.1:8000/api/patients/${p.id}/`, p)
            .then(() => {
                setEditingField({ id: null, field: null });
                fetchPatients(searchTerm, activeTab);
                fetchCounts();
            })
            .catch(() => alert("Грешка!"));
    };

    const handleSubmit = (isUrgent = false) => {
        const url = isUrgent ? 'http://127.0.0.1:8000/api/patients/add_urgent/' : 'http://127.0.0.1:8000/api/patients/';
        axios.post<Patient>(url, formData).then((res) => {
            setLastSuccess(res.data);
            fetchPatients(searchTerm, activeTab);
            fetchCounts();
            setFormData({ first_name: '', last_name: '', phone: '', diagnosis: '', operation: null, embg: '' });
            nameRef.current?.focusFirst();
            setTimeout(() => setLastSuccess(null), 30000);
        }).catch(() => alert("Грешка!"));
    };

    return (
        <div className="full-screen-wrapper">
            <aside className="sidebar-admin shadow">
                <h3 className="panel-title">Нов Пациент</h3>
                {lastSuccess && (
                    <div className="alert alert-success shadow-sm p-3 mb-3 border-start border-5 border-success position-relative">
                        <button type="button" className="btn-close position-absolute top-0 end-0 p-2" onClick={() => setLastSuccess(null)} style={{fontSize: '0.7rem'}}></button>
                        <h6 className="fw-bold mb-1">УСПЕШНО:</h6>
                        <div className="mb-2 small">{lastSuccess.first_name} {lastSuccess.last_name}</div>
                        <div className="d-flex justify-content-between bg-white p-2 rounded border">
                            <div className="text-center px-2">
                                <small className="d-block text-muted" style={{fontSize: '0.65rem'}}>РЕД БРОЈ</small>
                                <span className="fw-bold text-primary fs-5">{lastSuccess.position}</span>
                            </div>
                            <div className="text-center px-2 border-start">
                                <small className="d-block text-muted" style={{fontSize: '0.65rem'}}>КОД</small>
                                <span className="fw-bold text-dark fs-5">{lastSuccess.access_code}</span>
                            </div>
                        </div>
                    </div>
                )}
                <form className="admin-form" onSubmit={(e) => e.preventDefault()}>
                    <NameFields ref={nameRef} firstName={formData.first_name} lastName={formData.last_name}
                        onFirstChange={(v) => setFormData({...formData, first_name: v})}
                        onLastChange={(v) => setFormData({...formData, last_name: v})}
                        onNext={handleNextField} />
                    <PhoneInput value={formData.phone} onChange={(v) => setFormData({...formData, phone: v})} onNext={handleNextField} />
                    <OperationInput value={formData.operation} onChange={(v) => setFormData({...formData, operation: v})} onNext={handleNextField} />
                    <DiagnosisModule value={formData.diagnosis} onChange={(v) => setFormData({...formData, diagnosis: v})} onNext={handleNextField} />
                    <EmbgModule value={formData.embg} onChange={(v) => setFormData({...formData, embg: v})} onNext={handleNextField} />
                    <div className="d-flex gap-2 mt-3">
                        <button type="button" className="btn-submit flex-grow-1" onClick={() => handleSubmit(false)}>ВНЕСИ</button>
                        <button type="button" className="btn-urgent" onClick={() => handleSubmit(true)}>ИТНО !</button>
                    </div>
                    <div className="mt-5 pt-3 border-top border-secondary">
                        <button type="button" className="btn btn-outline-danger w-100 fw-bold" onClick={handleLogout}>ОДЈАВИ СЕ</button>
                    </div>
                </form>
            </aside>
            <main className="main-content-admin">
                <header className="content-header d-flex justify-content-between align-items-center">
                    <div className="tab-container-nav">
                        <div className="tab-menu">
                            <button className={`tab-link ${activeTab === 'call' ? 'active' : ''}`} onClick={() => setActiveTab('call')}>Листа за Повик ({counts.call})</button>
                            <button className={`tab-link ${activeTab === 'waiting' ? 'active' : ''}`} onClick={() => setActiveTab('waiting')}>Чекална ({counts.waiting})</button>
                            <button className={`tab-link ${activeTab === 'trash' ? 'active' : ''}`} onClick={() => setActiveTab('trash')}>Корпа ({counts.trash})</button>
                        </div>
                    </div>
                    <SearchInput value={searchTerm} onChange={setSearchTerm} />
                </header>
                <PatientTable 
                    type={activeTab} patients={patients} editingField={editingField} setEditingField={setEditingField} 
                    handleLiveEdit={handleLiveEdit} saveField={saveField} onAction={(p) => saveField(p)}
                />
            </main>
        </div>
    );
}

export default AdminPanel;